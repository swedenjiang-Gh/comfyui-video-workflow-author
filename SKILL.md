...
3. 提交 `/prompt` 后，使用 `D:\Comfy-Desktop\ComfyUI-Shared\scripts\Watch-ComfyUI-Progress.ps1 -PromptId <prompt_id>` 监听隐藏 API 的 `/ws` 事件，向用户显示当前节点和步数进度；不得因为 API 窗口隐藏而声称任务无进度。
