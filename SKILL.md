...
1. 搜索当前项目的授权角色卡、参考图、道具图和既有关键帧；先查短剧项目专属的 `D:\MediaStudio\AIShortDrama\Projects\<项目名>\` 与其权威清单，再查项目共用资产 `D:\MediaStudio\assets\`。优先选择同时匹配所需结构和镜头角度的素材；不得修改或覆盖原资产，也不得扫描无关磁盘目录。
